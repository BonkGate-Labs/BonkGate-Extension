// BonkGate - X/Twitter Blockchain Intelligence Extension

// Check if BonkGateAPI is defined
if (typeof BonkGateAPI === 'undefined') {
    console.error('BonkGateAPI is not defined. Make sure apiService.js is loaded before content.js');
    ...
    try {
        ...
        console.log('BonkGateAPI loaded dynamically');
    } catch (e) {
        console.error('Failed to load BonkGateAPI dynamically:', e);
    }
}
...
const FEATURES = [
    {
        id: 'contracts',
        name: 'Contracts',
        description: 'Even from deleted addresses, BonkGate finds contracts posted by users and projects.',
        icon: '📊',
        color: 'bg-purple-500'
    },
    {
    ...
    },
    {
        id: 'key-followers',
        name: 'Key Followers',
        description: 'Using our own custom scoring system, BonkGate highlights the most important followers and contributors based on their relevance and influence.',
        icon: '⭐',
        color: 'bg-orange-500'
    }
];
...
function addBonkGateButton() {
    // Only add button on profile pages and if not already present
    if (!window.location.pathname.match(/\/[^/]+$/) || document.querySelector('.bonkgate-btn')) {
        return;
    }
    ...
    if (!specificContainer) {
        ...
        console.log('Using fallback container for BonkGate button');
        insertBonkGateButton(fallbackContainer);
    } else {
        console.log('Found specific container for BonkGate button');
        insertBonkGateButton(specificContainer);
    }
    
    // Helper function to create and insert the button
    function insertBonkGateButton(container) {
        // Create the BonkGate button to match Twitter/X's UI
        const button = document.createElement('button');
        button.title = 'Analyze with BonkGate';
        button.type = 'button';
        
        // Use Twitter/X button styling
        button.className = 'css-1dbjc4n r-1777fci r-bt1l66 r-bztko3 r-lrvibr r-1loqt21 r-o7ynqc r-6416eg r-1ny4l3l bonkgate-btn';
        
        // Create icon
        const icon = document.createElement('img');
        icon.src = chrome.runtime.getURL('assets/raw.png');
        icon.alt = 'BonkGate';
        icon.className = 'bonkgate-icon';
        ...
        button.addEventListener('click', async (e) => {
            ...
            openBonkGateModal(username);
        });
    }
}

// Create and open the main BonkGate modal
function openBonkGateModal(username) {
    // Prevent multiple modals
    closeAllModals();
    
    // Create modal container with backdrop
    const modalContainer = document.createElement('div');
    modalContainer.className = 'bonkgate-modal-container fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50';
    modalContainer.dataset.username = username;
    
    // Create modal
    const modal = document.createElement('div');
    modal.className = 'bonkgate-modal relative glass-bg rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col text-black';
    ...
    title.textContent = `BonkGate Analysis: @${username}`;
    ...
    contentArea.className = 'flex-1 overflow-y-auto glass-bg p-6 min-h-0';
...
function loadFeatureContent(featureId, username, contentArea) {
    ...
    switch (featureId) {
        case 'contracts':
            apiPromise = BonkGateAPI.getContractTweets(username);
            break;
        case 'usernames':
            apiPromise = BonkGateAPI.getPastUsernames(username);
            break;
        case 'bio':
            apiPromise = BonkGateAPI.getBioHistory(username);
            break;
        case 'first-followers':
            apiPromise = BonkGateAPI.getFirstFollowers(username);
            break;
        case 'key-followers':
            apiPromise = BonkGateAPI.getScoredFollowers(username);
            break;
        default:
    ...
    apiPromise.then(data => {
        ...
        try {
            ...
            sessionStorage.setItem(`bonkgate_cache_${username}`, 'true');
        } catch (e) {
                ...
                <div class="contract-card">
                    ...
                    <div class="contract-body">
                        ...
                        <div class="tweet-message mb-3 text-gray-300 p-3 bg-gray-800 bg-opacity-50 rounded">
                            ${contract.deleted ? '<div class="text-orange-400 text-xs mb-1">This tweet has been deleted but was captured by BonkGate.</div>' : ''}
                            ${contract.message}
                        </div>` : ''}
...
function closeAllModals() {
    ...
    const modals = document.querySelectorAll('.bonkgate-modal-container');
...
function initialize() {
    // Ensure BonkGateAPI is available
    if (typeof BonkGateAPI === 'undefined') {
        console.error('BonkGateAPI is still not defined. Extension functionality will be limited.');
        return;
    }
    
    console.log('Initializing BonkGate extension');
    injectTailwindCSS();
